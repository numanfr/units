...
fn main(){
    ...
    let one_unit = Vector{value:DerivedQuantities::Scalar.get_value().set_magnitude(1_f64), theta:45_f64.to_radians()};
    let other_unit = Vector{value:DerivedQuantities::Scalar.get_value().set_magnitude(1_f64), theta:-45.0_f64.to_radians()};
    let res = other_unit + one_unit;
    println!("{:#?} {}",res, res.theta.to_degrees());


    let one_unit = Vector{value:DerivedQuantities::Scalar.get_value().set_magnitude(1_f64), theta:45_f64.to_radians()};
    let other_unit = Vector{value:DerivedQuantities::Scalar.get_value().set_magnitude(1_f64), theta:135.0_f64.to_radians()};
    let res = other_unit + one_unit;
    println!("{:#?} {}",res, res.theta.to_degrees());


    let a = Vector::get_angle(1_f64, 1_f64).to_degrees();
    let b = Vector::get_angle(-1_f64, 1_f64).to_degrees();
    let c = Vector::get_angle(-1_f64, -1_f64).to_degrees();
    let d = Vector::get_angle(1_f64, -1_f64).to_degrees();
